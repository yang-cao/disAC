...
datasets of \warn{16} GB for both benchmarks. 
...
\eetitle{Parametric queries}. We used built-in %parameterized
parameteric
queries of both benchmarks. When a query is not
\SPC, we used its \SPC sub-queries instead. %After this,
We
\warn{sampled 15} built-in parametric \SPC\ queries for each
benchmark. To evaluate with larger workloads and different
query complexities, we also
generated \warn{15} %and \warn{15} parametric \SPC
 parametric queries for each of \tpch and \tpcds,
%respectively,
where the number \#-\at{join} of joins varied from \warn{1} to
\warn{5};
\warn{in total, we had 6 queries for each \#-\at{join} %number of joins
and all of them are K-FK join only queries.}\looseness=-1
...
universe set $\kb{\A}$.
Instead, $f^{(b)}$ is to
%prioritize the performance of query evaluation over the \bdss while
...
on the size of $\Sigma$ \warn{for} procedure \decompose (see the remark
...
\etitle{(2) Algorithm \uscsel} that returns $\kb{\R}$ that consists
...
We used SparkSQL v\warn{2.4.0} over Cassandra v\warn{3.11.6} as the \kv
...
as a Spark master/slave for the SQL layer \warn{(with 4GB of memory)} and also holds a
Cassandra partition for the storage \warn{(also with 4GB of memory)}.
By default, we set the storage budget $B_{\at{max}}$ of the
\bdss to be \warn{0.7 times of the size of the \tpcds database schema,
