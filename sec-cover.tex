...
$\kb{\A}$ of \bss for $\Q$, as part of the input for \revise{algorithm
\opts above}. %in Section~\ref{sec-select}.

\stitle{Algorithm}. The algorithm, denoted by \usc, takes as input a workload $\Q$
over \revise{a} database schema $\R$, and returns a set $\kb{\A}$ of \bss
for $\Q$.
...
\eetitle{Step (1)}. It first generates two \bss for each \revise{query} $Q$:
$\kb{R}_{Q}^{1} = \bschema{P_{Q}}{Y_{Q}}$ and $\kb{R}_{Q}^{2} =
\bschema{P_{Q}Y_{Q}}{\emptyset}$. \revise{Here} $\{\kb{R}_{Q}^{1}\}$ is
...
\revise{of $\kb{\A}$} claimed, \ie $\kb{\A}_{Q}$ can cover all optimal
\bdss for $Q$, even when only measure (2) is \revise{concerned} alone. In
...
$Q$, yielding \bss that do not span \revise{across} multiple relations. The
process starts with the relation $R$ \revise{that contains} the maximum
...
It then identifies the relation $R'$ in $Q$ \revise{such that
  it} contains the maximum
number of attributes that are in \bss $\kb{R}_{Q}$ with $R$
marked processed, and \revise{handles} $R'$ along the same lines. The
...
\eetitle{Step (3)}. \revise{This final step} is to generate cross-relation \bss
...
generates again one \bs per each relation of $Q$ that is not \revise{yet}
in $\Sigma$ and one \bs $\kb{R}_{Q}^{\Sigma}$ for $R_{\Sigma}$. Note
that $\kb{R}_{Q}^{\Sigma}$ may \revise{span across} two relations in $\Sigma$.
The step \revise{is done} %finishes
...
\revise{Algorithm \usc is outlined as Algorithm~\ref{alg-usc}}.
\revise{Step (1) is carried out by} lines~\ref{usc-l1}-\ref{usc-l2}.
\revise{followed by 
  steps (2) and (3)} (lines~\ref{usc-l3}-\ref{usc-l5}).
In particular, when $\Sigma$
contains only a single relation of $Q$, \revise{a single execution
...
procedure \decompose, \revise{also shown}
...
\revise{for $\kb{\A}$, observe} that \usc computes $\kb{\A}$ consisting of
...
%\DontPrintSemicolon
%\SetAlgoLined
\KwIn{\revise{A database} schema $\R$ and a workload $\Q$ over $\R$.}
...
relation or \revise{across} two relations connected via K-FK join. While
this already guarantees property (a) of \usc, we can further
improve the coverage of the universe set $\kb{\A}$ returned by
\usc by covering all \revise{\bss across} more than two relations for each
...
this only \revise{increases}  
the complexity of \usc to
$O(2^{\kappa}\cd{\Q})$-time; where \revise{$\kappa$} %$2^{\kappa}$
is typically a
