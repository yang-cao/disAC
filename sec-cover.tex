...
%
When $Q$ is \revise{in \SQL}, we decompose it into its \SPC
...
any ranking function $f$ with \warn{$c_{4} = 0$}. The key observation
here is that $\kb{R}_{Q}^{1}$ constructed in step (1) already forms an
optimal \bds under both \warn{measures (1) and (3)}.
After expanding $\kb{\A}_{Q}$ with \bss over individual relations
in $Q$ generated by step (2), by the definition of \warn{measure (4)},
