...
$Q_{s}'$.
By treating \SQL with \revise{union/negation} as a set of max-\SPC
sub-queries, we ensure the ILP programs correctly characterize
value propagation across \revise{their attributes}.
\revise{This does not lose the generality}
since values only propagate within \SPC
sub-queries of \SQL; hence \revise{it suffices for
the ILP programs to} capture
all such propagation with \bss encoded by their feasible
answers to cover the optimal \bdss.
%
\eat{By treating \SQL with unions/negations as a set of max-\SPC
sub-queries, we ensure the ILP programs correctly characterize
value propagation across the attributes for them. 
Note that this is not a restriction 
since values only propagate within \SPC
sub-queries of \SQL; hence 
the ILP programs need only to capture
all such propagation with \bss encoded by their feasible
answers to cover the optimal \bdss.}
...
\revise{Intuitively}, (1) and (2) of Lemma~\ref{lem-ILP} 
provide a
...
%
\eat{Note that (1) and (2) of Lemma~\ref{lem-ILP} give a
necessary condition and sufficient condition, respectively, for
$Q$ being scan-free over the selected \bds $\kb{\R}$.
Lemma~\ref{lem-ILP}(3) is a sufficient and necessary
characterization for $Q$ being strongly scan-free over $\kb{\R}$.}
