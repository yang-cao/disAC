\vspace{2ex}
%%%%%%%% Section 4 %%%%%%%%
\section{\warn{Computing Optimal \textsf{BaaV} Schemas}}
\label{sec-select}

In light of Theorem~\ref{thm-complexity}, any efficient solution
to \revise{\baav schema selection} is necessarily approximate.
This said, below we develop a practical algorithm 
that given \revise{any ``universe'' set} $\kb{\A}$ of \bss,
\revise{computes} the optimal \bds $\kb{\R}$ for $\Q$ that is subsumed by
$\kb{\A}$. \revise{That is, it is able to select the optimal $\kb{\R}$ 
from  $\kb{\A}$ whenever $\kb{\A}$ covers sufficient \bss
including the optimal ones}.


Below we present the algorithm \revise{assuming a} given universe set $\kb{\A}$.
...
%\DontPrintSemicolon
%\SetAlgoLined
\KwIn{$\Q$ over $\R$, \revise{a set}
  $\kb{\A}$ of \bss, rank aggregate function $f$.}
\KwOut{\bds $\kb{\R}$ in \revise{the normal} form for $Q$ with minimum $f(\Q,
...
\stitle{Algorithm}.
The \revise{main driver of the} algorithm, denoted by \opts,
\revise{is} shown as
...
\ie for any other \bds $\kb{\R}'\ak \subseteq \ak \kb{\A}$ \revise{in the}
...
\revise{capitalize on} the latest advances of ILP, \eg algorithms
(cf.~\cite{ILPbook}) and commercial ILP
solvers~\cite{cplex,gurobi,XpressMP}. \revise{We use
...
\revise{highlights} strongly scan-free queries, \ie $\kb{\R}$ is optimal
among all normalized \bdss when $f(\Q, \kb{\R})$ \revise{revises
  criterion (1) such that} $\sf(Q,\ak \kb{\R}) \ak = \ak 1$ \revise{if
  and only if
  (iff)} $Q$ is strongly scan-free over $\kb{\R}$
\revise{(Section~\ref{sec-schema})}.
An \SPC
query $Q$ is {\em a key-foreign key (K-FK) join query} if all joins in
$Q$ are K-FK joins.
\revise{The {\em acyclicity} of $\Q$ concerns dependencies
in a schema graph, which will be elaborated later on.}\looseness=-1

\vspace{0.6ex}
\revise{Theorem~\ref{thm-opts} tells us that the \baav schema
$\kb{\R}$ computed by
...
\vspace{0.6ex}
\revise{We give a constructive proof for Theorem~\ref{thm-opts}
...
\revise{Based on its answers \opts} decides whether $\Q$ is acyclic,
and returns the \bds encoded in the solution to ILP-(I) if so
(line~\ref{opts-l2}); \revise{the \bds} is the optimal normalized \bds from
$\kb{\A}$ for \revise{K-FK join \SPC queries}
...
\vspace{0.8ex}
Below we present the two ILP programs and \revise{the identification
of acyclic queries}. \revise{We then give a proof
...
$z_{Q} = 1$ \revise{iff} we pick \bss for $\kb{\R}$ over which $\Q$ is scan-free.
(b)~For each query $Q\in \Q$ and attribute $A$ in $Q$, 
$x_{A}^{Q}$ indicates whether $A$ is \revise{``fetchable'' (see below)}
from constants
in $Q$ over $\kb{\R}$. 
...
attributes \revise{appearing} %appeared
in the predicates of $Q$ that
\revise{are in the form of relational}
algebra (\ie $Y_{Q}$ includes attributes \revise{that appear} %appeared
in joins or project predicates of $Q$). 
\looseness = -1

\vspace{0.36ex}
For each $Q$ in $\Q$ and each relation \revise{atom} $R$ in $Q$, denote by
...
\revise{Below we give intuition behind the ILP program.
...
$Q$ (\ie $X_{A}^{Q} = 1$), $A$ must \revise{be either in $P_{Q}$ or 
...
\vspace{0.8ex}
\revise{When taken together, 
ILP constraints (2)-(6)} ensure that if $z_{Q}\ak
=\ak 1$, \ie~\revise{if} $Q$ is scan-free over \bss $\kb{R}$ picked for $Q$
($y_{\kb{R}}^{Q}\ak =\ak 1$), \revise{then} for each attribute $A$ in $Y_{Q}$,
there must exist a sequence of \get operations that can retrieve
$A$ over the picked \bss starting from parameters $P_{Q}$ of $Q$
(assuming \revise{that} the picked \bss are ``acyclic'', which we will
elaborate shortly).
...
ILP program must yield a \bds that is in \revise{the} normal form for all
...
\revise{by using existing ILL solvers, \eg Gurobi~\cite{gurobi} or
...
solution. We \revise{refer to such $\kb{\R}$ {\em the \bds encoded by the
...
\revise{Continuing} with Example~\ref{exa-measures}, 
...
\stitle{(2) Acyclicity checking}. \revise{Below we} 
...
\revise{Continuing with Example~\ref{exa-ILP-I}, let}
...
thus $f_{0}(\Q, \kb{\R}_{4})$ is \revise{inconsistent} with the
objective of ILP-(I).
...
fetched from parameters $P_{Q}$ of $Q$ even \revise{when} $x_{A}^{Q} = 1$,
...
In other words, ILP-(I) \revise{alone} cannot capture the scan-free
...
\revise{{\em cyclic}}. Query $Q$ is {\em acyclic} \wrt $\kb{\R}_{o}$ if
...
for such cases, \revise{\opts further triggers} ILP-(II) below.


\vspace{0.36ex}

\stitle{(3) ILP-(II)}.
\revise{For cyclic queries,
...
\revise{$k$'s} are the parameters of $Q$. 
%\looseness = -1
...
\begin{lemma}\label{lem-ILP-norm}
  ...
  $\kb{\R}$ is in \revise{the} normal form for $Q$. 
\end{lemma}
...
by inequation (8) for each \revise{query $Q$} in $\Q$.
Hence, by Lemma~\ref{lem-ILP-norm}, any feasible solution to ILP-(I)
or ILP-(II) must encode a \bds that is in \revise{the} normal form for $\Q$.
This verifies Theorem~\ref{thm-opts}(1). 


\vspace{1ex}
\revise{For feasible solutions to ILP-(I) and ILP-(II) and the
...
for each $A\in Y_{Q}$. One can \revise{verify} by induction on the length
of the chain that each chain encodes a partial assignment to the
0/1-variables that satisfies \revise{the} constraints of ILP-(I) and leads to
$x_{A}^{Q} = 1$ for each $A\in Y_{Q}$. By inequation (3), such
partial assignments, together with $z_{Q} = 1$, form a feasible
solution (\ie assignment) that satisfies all \revise{the} constraints
in ILP-(I).
%
\revise{One can verify
...
\revise{(resp.~strongly)} scan-free if and only if $\bschema{P_{Q}}{Y_{Q}}$ is
implied by $\kb{\R}$, \ie there exists a \revise{(resp.~strongly)} scan-free
...
\revise{Putting these together, we are now ready to prove
  Theorem~\ref{thm-opts}.}

\vspace{0.8ex}
\noindent
    {\bf Proof of Theorem~\ref{thm-opts}}. \revise{As remarked earlier,
...
must have \revise{the} minimum ranking score.
%, although its ``true''
...
\vspace{0.6ex}
\revise{For Theorem~\ref{thm-opts}(2b)}, 
...
(c) ILP-(II) \revise{and ILP-(I) have} at most
...
\sstab (1) One can \revise{treat some of the measures of \opts}
as constraints subject to given budgets. For instance, if \revise{users
want} to find optimal normalized \bds with size bounded by a
storage budget $B$, then \revise{they} can simply move $f_{3}$ from the
...
$f(\Q, \kb{\R})$, one can \revise{prioritize certain}
measures. For example, if \revise{users care more about the
  performance of query answering than}
  %while downplaying
the maintenance cost, \revise{\eg when querying static historical data},
they could assign $c_{1}$ a
  large coefficient, %\revise{followed by a moderate $c_{2}$},
  and set $c_{4}$
to 0.
