...
more data %values
from $\kb{\at{T}}_{1}'$:
\warn{with $\kb{\at{T}}_{1}'$ it fetches \at{transaction}
...
%%%%%%%%
\stitle{Contribution \& organization}. This paper investigates %tackles
...
by our algorithms. With SparkSQL-over-Cassandra (\soc), on average \warn{\tpch and
...
(b) \warn{Our algorithms are effective.
Over \tpch and \imdb,  \soc is 4.28 and \xx times
faster with \baav schemas generated by our algorithms than with
...
\revise{Revise after new experiments. New benchmarks of \SQL?}

\vspace{0.6ex}
\warn{All formal details and proofs can be found in the full
version~\cite{full}.}
